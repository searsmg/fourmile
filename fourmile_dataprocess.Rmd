...
prec_update <- prec %>%
  ...
  mutate(Gordon_South = ifelse(!doy %in% c(152:258), 
         ...
         NADP_SL = ifelse(!doy %in% c(152:258), 
                               NA, NADP_SL)) %>%
  mutate_at(c('prism_W', 'prism_M', 'prism_E'), funs(lead), n = 1) #%>%
  mutate_at(c('prism_W', 'prism_M', 'prism_E'), )
...
```{r}
niwot_prism <- read_csv(here('data', 'niwot_prism_compare.csv')) 

ni_prism <- niwot_prism %>%
...
niwot_compare <- full_join(ni_prism, niwot_snotel, by = 'date')
