...
t_moplot <- ggplot(catchment_mo2, aes(x = month_name, 
  ...
  scale_fill_manual(values = c("#0047AB", "orange"),
                    labels = c("Gridded", "Station")) +
  theme_bw(base_size=20) +
  labs(y = expression("Tair " ( degree~C)),
       x = element_blank()) +
...
Percent difference between station and gridded (tmean, solrad, RH)

```{r}

# monthly percent difference (symmetric): (statn - grid) / mean(statn, grid) * 100
pct_diff_mo <- catchment_mo_mean %>%
  pivot_wider(names_from = site, values_from = c(tmean_mean, solrad_mean, rh_mean)) %>%
  mutate(
    tmean_pct_diff = (tmean_mean_statn - tmean_mean_grid) /
      ((tmean_mean_statn + tmean_mean_grid) / 2) * 100,
    solrad_pct_diff = (solrad_mean_statn - solrad_mean_grid) /
      ((solrad_mean_statn + solrad_mean_grid) / 2) * 100,
    rh_pct_diff = (rh_mean_statn - rh_mean_grid) /
      ((rh_mean_statn + rh_mean_grid) / 2) * 100
  ) %>%
  mutate(month_name = factor(month.abb[month], levels = month.abb)) %>%
  dplyr::select(month, month_name, tmean_pct_diff, solrad_pct_diff, rh_pct_diff) %>%
  arrange(month)

pct_diff_mo

write_csv(pct_diff_mo,
          './data/monthly_percent_diff.csv')

# overall percent difference across all months
pct_diff_overall <- pct_diff_mo %>%
  summarize(across(c(tmean_pct_diff, solrad_pct_diff, rh_pct_diff),
                    ~ mean(.x, na.rm = TRUE)))

pct_diff_overall

write_csv(pct_diff_overall,
          './data/overall_percent_diff.csv')

# annual total precip by site and year
annual_precip <- catchment_mo_sum %>%
  group_by(site, year) %>%
  summarize(p_annual = sum(p_sum, na.rm = TRUE)) %>%
  ungroup()

annual_precip

write_csv(annual_precip,
          './data/annual_precip_by_year.csv')

# average annual precip by site (across all years)
avg_annual_precip <- annual_precip %>%
  group_by(site) %>%
  summarize(p_annual_mean = mean(p_annual, na.rm = TRUE))

avg_annual_precip

write_csv(avg_annual_precip,
          './data/avg_annual_precip.csv')
...
annotate(
  "point",
  x = as.Date("2013-09-11"),
  y = 0.8,
  shape = 1,
  size = 6,
  stroke = 1.2,
  color = 'red'# controls the thickness of the outline line
) +
  theme_bw(base_size = 20) +
  labs(y = 'Streamflow (cms)', x = element_blank()) +
...
# mean and peak for each year
flow_stats <- flow %>%
  group_by(year) %>%
  summarize(mean = mean(obs, na.rm=T),
...
ggsave("./figures/figure2_wlabels_sep2026.png", 
       plot = quad_plus, 
       dpi = 600,
       width = 14,
       height = 12,
...
ggsave(filename = "./figures/figure3_aug2026.jpg",
       plot = final_plot,
       dpi = 800,
       bg = 'white',
       width = 12,
...
flow_plot <- read_csv('./38HRU_oct2025_updates/all_kge.csv') %>%
  filter(metric == 'KGE') %>%
  dplyr::select(-metric) %>%
  rename(kge = value)
...
# Overall KGE
flow_plot_updated %>% 
  filter(!year_excluded == 'All') %>% 
  group_by(source) %>%
  summarize(mean_kge = mean(kge))
...
pbiasCats <- read_csv('./38HRU_oct2025_updates/pbiasCats.csv') #%>%
...
# Overall KGE
pbiasCats %>% 
  filter(!year_missing == 'All') %>% 
  group_by(source) %>%
  summarize(mean_pbias = mean(pbias))
...
ggsave('./figures/figure4_PbiasLegend.png',
       dpi=800,
       width = 12,
       height = 8,
       bg = 'white')
...
nseCats <- read_csv('./38HRU_oct2025_updates/nseCats.csv') #%>%
  #rename(source = input_data)
...
tight_theme <- theme(
  ...
  plot.margin = margin(t = 14, r = 4, b = 2, l = 14),   # more left margin for the tag
  legend.key.size = unit(0.4, "cm"),
  legend.spacing.y = unit(0.05, "cm"),
  legend.box.margin = margin(0, 0, 0, 0),
  legend.margin = margin(t = 0, b = 0, l = 2, r = 0, unit = "pt"),
  plot.tag = element_text(size = 14, face = "bold"),
  plot.tag.position = "topleft"     # <- keyword instead of numeric coords; avoids clipping
)

no_xaxis_theme <- theme(
...
# =========================================================
# KGE panel (top — keeps facet strip labels)
# =========================================================
...
flow_plot_updated <- flow_plot %>%
  mutate(source = case_when(
    ...
    grepl("statn", model_run) ~ "statn"),
    year_excluded = str_extract(model_run, "\\d{4}$")) %>%
  mutate(year_excluded = if_else(model_run %in% c('gridall', 'statnall'),
                                  'None', year_excluded)) %>%
  mutate(kge_revise_bins = cut(kge,
                               breaks = c(-Inf, 0.0, 0.2, 0.4, 0.6, 0.8, 1),
                               include.lowest = TRUE,
...
both_kge_plot <- flow_plot_updated %>%
  ggplot(aes(x = year, y = year_excluded, fill = kge_revise_bins)) +
  geom_tile(color = "white") +
  facet_wrap(~source, labeller = labeller(source = new_facet_titles)) +
  scale_fill_manual(values = c(
    "< 0.0" = "#DCBE9BFF",
    ...
    "> 0.8" = "#009474FF"),
    guide = guide_legend(ncol = 1)) +
  geom_text(aes(label = sprintf("%.2f", kge), color = kge_revise_bins), size = 3) +
  scale_color_manual(values = c(
    "< 0.0" = "black", "0.0-0.2" = "black", "0.2-0.4" = "black",
    "0.4-0.6" = "black", "0.6-0.8" = "black", "> 0.8" = "white"
  ), guide = "none") +
  theme_bw(base_size = 18) +
  labs(y = "Year excluded", x = "Year", fill = "KGE", tag = "a) KGE") +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
    legend.position = 'right',
    legend.text = element_text(size = 14),
  ...
  tight_theme +
  no_xaxis_theme
  # strip labels intentionally kept on this panel

# =========================================================
# Pbias panel (middle — no strip labels, no x-axis)
# =========================================================

pbiasCats <- read_csv('./38HRU_oct2025_updates/pbiasCats.csv')

both_pbias_plot <- pbiasCats %>%
  ggplot(aes(x = year, y = year_missing, fill = pbiasCat)) +
  geom_tile(color = "white") +
  facet_wrap(~source, labeller = labeller(source = new_facet_titles)) +
  scale_fill_manual(
    breaks = c("Very good", "Good", "Satisfactory", "Unsatisfactory"),
    values = c(
      ...
      "Good"           = "#72E1E1FF",
      "Very good"      = "#009474FF")) +
  geom_text(aes(label = sprintf("%0.1f", pbias), color = pbiasCat), size = 2.75) +
  scale_color_manual(values = c(
    "Unsatisfactory" = "black", "Satisfactory" = "black",
    "Good" = "black", "Very good" = "white"
  ), guide = "none") +
  theme_bw(base_size = 18) +
  labs(y = "Year excluded", x = "Year", fill = "Pbias", tag = "b) Pbias") +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
    legend.position = 'right',
    legend.text = element_text(size = 14),
...
# =========================================================
# NSE panel (bottom — keeps x-axis, no strip labels)
# =========================================================

nseCats <- read_csv('./38HRU_oct2025_updates/nseCats.csv')

both_nse_plot <- nseCats %>%
  ggplot(aes(x = year, y = year_missing, fill = nseCat)) +
  geom_tile(color = "white") +
  facet_wrap(~source, labeller = labeller(source = new_facet_titles)) +
  scale_fill_manual(
    breaks = c("Very good", "Good", "Satisfactory", "Unsatisfactory"),
    values = c(
      ...
      "Good"           = "#72E1E1FF",
      "Very good"      = "#009474FF")) +
  geom_text(aes(label = sprintf("%.2f", nse), color = nseCat), size = 2.75) +
  scale_color_manual(values = c(
    "Unsatisfactory" = "black", "Satisfactory" = "black",
    "Good" = "black", "Very good" = "white"
  ), guide = "none") +
  theme_bw(base_size = 18) +
  labs(y = "Year excluded", x = "Year", fill = "NSE", tag = "c) NSE") +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
    legend.position = 'right',
    legend.text = element_text(size = 14),
...
combined_fig <- both_kge_plot / both_pbias_plot / both_nse_plot +
  plot_layout(heights = c(1, 1, 1))

combined_fig

ggsave('./figures/figure4_combined.png',
       plot = combined_fig,
       dpi = 800,
       width = 14,
       height = 18,
...
perf <- perf %>%
  mutate(performance = case_when(
                                ...
                                nse < 0.5 & abs(pbias) < 25 ~ 'Pbias satisfactory',
...
#             stroke = 1.3) +
# scale_color_manual(values = c(
# "% bias satisfactory" = "#2E8B57", 
# "NSE + % bias satisfactory" = "black" 
# )) +
#   scale_fill_manual(values = c("#0047AB", "orange"),
#                     guide='none') +
#     theme(
...
#   scale_y_continuous(expand = expansion(mult = c(0.1, 0.25)))
# 
# ggsave('./figures/figure5_aug2026.png',
#        dpi=600,
#        width=12,
#        height=8,
#        bg='white')
...
ggplot(param_sens, aes(x=input_data, y=value, fill=input_data)) +
  ...
  scale_fill_manual(values = c("#0047AB", "orange"),
                    guide='none') +
  new_scale_fill() +
  geom_jitter(aes(fill = performance, color = performance),
              shape = 21,
              position = position_jitter(width = 0.2), 
              size = 4.5, 
              alpha = 0.9,
              stroke = 1.3) +
  scale_fill_manual(values = c(
    "Pbias satisfactory" = "white", 
    "NSE + Pbias satisfactory" = "black" 
  )) +
  scale_color_manual(values = c(
    "Pbias satisfactory" = "black", 
    "NSE + Pbias satisfactory" = "white" 
  )) +
  theme(
    legend.position = 'bottom',
    legend.text = element_text(size = 16),
...
ggsave('./figures/figure5_sep2026.png',
       dpi=600,
       width=12,
       height=8,
       bg='white')
...
# Shapiro-Wilk test per param
param_sens %>%
  group_by(param) %>%
  summarise(shapiro_p = shapiro.test(value)$p.value, n = n())

# Faceted Q-Q plots
ggplot(param_sens, aes(sample = value)) +
  stat_qq() +
  stat_qq_line() +
...
ggsave('./figures/figure6_august2026.png',
       dpi=600,
       width = 12,
       height = 8)
...
# simple for defense
flow_box <- ggplot(gw_maxcumsum, aes(x = source, y = flow_cms, fill = source)) +
  geom_boxplot(outliers=T,width=0.7) + # need to note 1 outlier is removed from RD1 grid and statn (2013 flood) ****
  facet_wrap(~flow_type, 
             scales = 'free',
             labeller = labeller(flow_type = facet_labels)) +
  labs(y = 'mm of water') + 
...
ggsave('./figures/figure6SIMPLE_oct2025.png',
       dpi=600,
       width = 12,
       height = 8)
...
gw_maxcumsum %>%
  group_by(flow_type) %>%
  summarise(shapiro_p = shapiro.test(flow_cms)$p.value, n = n())

# Faceted Q-Q plots
ggplot(gw_maxcumsum, aes(sample = flow_cms)) +
  stat_qq() +
  stat_qq_line() +
...
ggsave('./figures/figure7_aug2026.png',
       width = 11,
       height = 6,
       dpi=800,
       bg='white')
...
flowEg <- flow_pivot %>%
  ...
  mutate(cal_run = factor(cal_run, levels = c("Observed", "Gridded", "Station"))) %>%
  ggplot(aes(x = date,
             y = q_m3s, 
             color = cal_run,
             linetype = cal_run)) + 
...
# facet_labels <- c('RD1cumsum' = 'b) Surface runoff',
#                   'RD2cumsum' = 'c) Interflow',
#                   'RG1cumsum' = 'd) Fast groundwater',
#                   'RG2cumsum' = 'e) Slow groundwater')
# 
...
#   geom_vline(xintercept = as.Date("2016-04-01"),
#              linetype = "dashed",
#              color = "gray40",
#              linewidth = 0.4) +
#   scale_color_manual(values = c("#0047AB", "orange")) +
#     scale_x_date(date_labels = "%b",
#                date_breaks = "2 month") +
#   coord_cartesian(xlim = c(as.Date("2016-01-01"), as.Date("2016-09-22"))) +
#   labs(x=element_blank(),
#        y='mm of water') +
...
#           label_size = 16)
# 
# ggsave('./figures/figure8_july2026.png',
#        dpi=800,
#        width=10,
#        height=8)

#####
flow_types <- c("RD1cumsum", "RD2cumsum", "RG1cumsum", "RG2cumsum")
facet_labels <- c('RD1cumsum' = 'Surface runoff',
                  'RD2cumsum' = 'Interflow',
                  'RG1cumsum' = 'Fast groundwater',
                  'RG2cumsum' = 'Slow groundwater')

make_flow_plot <- function(ft) {
  ggplot(example %>% filter(flow_type == ft),
         aes(x = date, y = flow_cms, color = source)) +
    geom_line(size = 1, alpha = 0.7) +
    geom_vline(xintercept = as.Date("2016-04-01"),
               linetype = "dashed",
...
flow_plots <- lapply(flow_types, make_flow_plot)

bottom_row <- plot_grid(plotlist = flow_plots,
                         nrow = 2,
                         labels = c("b)", "c)", "d)", "e)"),
                         label_size = 16)
...
ggsave('./figures/figure8_aug2026.png',
       dpi=800,
       width=10,
       height=8)
...
combo <- read_csv('./38HRU_oct2025_updates/params_burn_unburn_update.csv')
...
# 
# ggsave('./figures/figure9_aug2026.png',
#        dpi=800,
#        width = 12,
#        height = 8)
...
ggplot(combo, aes(x = burn, y = value, fill = input_data)) +
  ...
  scale_fill_manual(values = c("#0047AB", "orange")) +
  new_scale_fill() +
  geom_jitter(aes(fill = performance, color = performance, group = input_data),
              shape = 21,
              position = position_jitterdodge(
                jitter.width = 0.3,
                dodge.width = 0.75),
              ...
              stroke = 1) +
  scale_fill_manual(values = c(
    "% bias satisfactory" = "white", 
    "NSE + % bias satisfactory" = "black" 
  )) +
  scale_color_manual(values = c(
    "% bias satisfactory" = "black", 
    "NSE + % bias satisfactory" = "white" 
  )) +
  facet_wrap(~param, 
             scales = 'free_y',
             strip.position = 'left') +
...
runoff <- both %>%
  ...
  mutate(runoff_cum = cumsum(outRD1))
...
p_plot <- ggplot(p, aes(x = date,
                        ...
                        color = type)) +
  geom_line(size=1, alpha=0.8) +
    scale_color_manual(values = c("Burned" = "red", 
                                  "Unburned" = "darkblue")) +
  scale_x_date(date_labels = "%b") +
  labs(x = element_blank(),
       y = 'Precipitation (mm)') +
...
inter <- inter %>%
  mutate(type = factor(type, levels = c("Unburned", "Burned")))
...
surface_runoff_plot <- ggplot(runoff, aes(x = date, color= type,
                        y = runoff_cum)) +
  geom_line(size=1, alpha=0.7) +
    scale_color_manual(values = c("Burned" = "red", 
                                  "Unburned" = "darkblue")) +
...
# rebuild p_plot here first, then:
legend <- cowplot::get_plot_component(p_plot, "guide-box", return_all = TRUE)
class(legend)  # should NOT be NULL now
...
surface_runoff_plot <- surface_runoff_plot + theme(legend.position = "none")
...
ggsave('./figures/figure10_aug2026.png',
       dpi=800,
       bg='white',
       height=10,
       width=12)
