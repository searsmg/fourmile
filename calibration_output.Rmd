...
catchment_mo_max <- catchment %>%
  mutate(month = month(time)) %>%
  group_by(site, month) %>%
  summarise(across(precip:solRad, ~ max(.x, na.rm = TRUE))) %>%
  dplyr::select(site, month,
                precip_max = precip, tmax_max = tmean, 
                solrad_max = solRad, rh_max = rhum)

catchment_mo_min <- catchment %>%
  mutate(month = month(time)) %>%
  group_by(site, month) %>%
  summarise(across(precip:solRad, ~ min(.x, na.rm = TRUE))) %>%
  dplyr::select(site, month,
                precip_min = precip, tmin_min = tmean, 
                solrad_min = solRad, rh_min = rhum)


catchment_mo <- left_join(catchment_mo_mean, catchment_mo_max, 
                          by = c('month', 'site'))

catchment_mo2 <- left_join(catchment_mo, catchment_mo_min, 
                          by = c('month', 'site')) %>%
...
p_moplot <- ggplot(catchment_mo2, aes(x = month_name, y = precip_mean, 
                                      color = site, group = site)) +
  geom_ribbon(aes(ymin = precip_min, ymax = precip_max, fill = site), alpha = 0.2) +
  geom_line() +
  geom_point() +
  theme_bw() +
...
t_moplot <- ggplot(catchment_mo2, aes(x = month_name, y = tmean_mean, 
                                      color = site, group = site)) +
  geom_ribbon(aes(ymin = tmin_min, ymax = tmax_max, fill = site), alpha = 0.2) +
  geom_line() +
  geom_point() +
  theme_bw() +
...
sol_moplot <- ggplot(catchment_mo2, aes(x = month_name, y = solrad_mean, 
                                      color = site, group = site)) +
  geom_ribbon(aes(ymin = solrad_min, ymax = solrad_max, fill = site), alpha = 0.2) +
  geom_line() +
  geom_point() +
  theme_bw() +
  labs(y = 'Solar Radiation (unit)') +
  ggtitle('D')
...
ggarrange(p_moplot+ rremove("x.text") +
               rremove("x.title") +
               rremove('x.ticks'),
             t_moplot+ rremove("x.text") +
               rremove("x.title")+
               rremove('x.ticks'), 
             sol_moplot + rremove("x.text") +
               rremove("x.title")+
               rremove('x.ticks'),
             rh_moplot,
          common.legend = T,
