...
gw_grid <- grid %>%
  dplyr::select(time, genRD1, genRD2, genRG1, genRG2) %>%
  mutate(source = 'grid')
...
gw <- gw %>%
  ...
  mutate(RD1cumsum = cumsum(genRD1),
         RD2cumsum = cumsum(genRD2),
         RG1cumsum = cumsum(genRG1),
         RG2cumsum = cumsum(genRG2))

ggplot(gw, aes(x=date, color = source)) +
  geom_line(aes(y=RD1cumsum), size =1) +
  facet_wrap(~year, scales = 'free', ncol=2) +
  scale_x_date(labels = date_format("%b")) +
  ggtitle('Cumulative genRD1') +
  theme(text = element_text(size = 20))   
