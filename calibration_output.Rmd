...
pbias_plot_mo <- ggplot(mo_pivot2, aes(month, pbias, color = cal_run)) + geom_point(size = 3) +
  theme_bw()
...
plot_cumulq_yr_ensem <- function(calyear) {
  {ensem_cumq %>%
    ...
    ggplot(aes(x = date,
               ...
               color = cal_run)) +
    geom_line() +
    geom_line(data=ensem_cumq[ensem_cumq$cal_run=='obs',], aes(group=cal_run), color = "black") +
    ggtitle(calyear) +
    theme_bw() +
