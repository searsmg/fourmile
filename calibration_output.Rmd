...
tmean7_grid <- read_csv('C:/Users/sears/Documents/Repos/fourmile/HRU_output/HRU_7_grid.csv') %>%
  dplyr::select(date, tmean) %>%
  rename(t7_grid = tmean)

tmean7_statn <- read_csv('C:/Users/sears/Documents/Repos/fourmile/HRU_output/HRU_7_statn.csv') %>%
  dplyr::select(date, tmean) %>%
  rename(t7_statn = tmean)

tmean9_grid <- read_csv('C:/Users/sears/Documents/Repos/fourmile/HRU_output/HRU_9_grid.csv') %>%
  dplyr::select(date, tmean) %>%
  rename(t9_grid = tmean)

tmean9_statn <- read_csv('C:/Users/sears/Documents/Repos/fourmile/HRU_output/HRU_9_statn.csv') %>%
  dplyr::select(date, tmean) %>%
  rename(t9_statn = tmean)

tmean22_grid <- read_csv('C:/Users/sears/Documents/Repos/fourmile/HRU_output/HRU_22_grid.csv') %>%
  dplyr::select(date, tmean) %>%
  rename(t22_grid = tmean)

tmean22_statn <- read_csv('C:/Users/sears/Documents/Repos/fourmile/HRU_output/HRU_22_statn.csv') %>%
  dplyr::select(date, tmean) %>%
  rename(t22_statn = tmean)

tmean36_grid <- read_csv('C:/Users/sears/Documents/Repos/fourmile/HRU_output/HRU_36_grid.csv') %>%
  dplyr::select(date, tmean) %>%
  rename(t36_grid = tmean)

tmean36_statn <- read_csv('C:/Users/sears/Documents/Repos/fourmile/HRU_output/HRU_36_statn.csv') %>%
  dplyr::select(date, tmean) %>%
...
## Cumulative Tmean difference

2011-2020 average of daily station-grid Tmean for DOY 365 (or 366)

```{r cumulative tmean for HRUs, echo = F}
#grid first
setwd("C:/Users/sears/Documents/Repos/fourmile/HRU_output/grid")
...
#statn next
setwd("C:/Users/sears/Documents/Repos/fourmile/HRU_output/statn")
...
tmean_hrus <- left_join(hru_grid_tmean, hru_statn_tmean, by = c('time', 'ID')) %>%
  mutate(diff = tmean_statn - tmean_grid,
         year = year(time)) %>%
  group_by(year, ID) %>%
  mutate(total_diff = cumsum(diff)) %>% 
  filter(time == max(time)) %>%
  group_by(ID) %>%
  summarize(tmean_avg_cumdiff = mean(total_diff)) %>%
  rename(value = ID)
...
tm_shape(hrus_map_tmean) +
  tm_polygons('tmean_avg_cumdiff', id = 'value') +
tm_shape(locations_t) +
  tm_symbols(col='blue', size=4)

```

Relative Tmean difference

```{r relative tmean, echo = F}

reg_tmean <- read_csv('C:/Users/sears/Documents/Repos/fourmile/HRU_output/reg_tmean.csv',
                      skip = 13) %>%
  dplyr::select(-starts_with('@')) %>%
...
hrus_map_tmean <- left_join(hrus_map_tmean, reg_tmean, by = 'value') 

hrus_map_tmean <- hrus_map_tmean%>%
  mutate(relative_avgcum_diff = tmean_avg_cumdiff / tmean_avg)

tm_shape(hrus_map_tmean) +
  tm_polygons('relative_avgcum_diff', id = 'value') +
tm_shape(locations_t) +
  tm_symbols(col='blue', size=4)
...
Relative Precip difference

```{r relative p, echo = F}

reg_p <- read_csv('C:/Users/sears/Documents/Repos/fourmile/HRU_output/reg_precipAll.csv',
                      skip = 21) %>%
  dplyr::select(-starts_with('@')) %>%
  mutate(time = dmy(time)) %>%
  summarize(across(`3`:`19`, ~ mean(.))) %>%
...
## Cumulative solar radiation difference

2011-2020 average of daily station-grid solar rad for DOY 365 (or 366)
...
solrad_hrus <- left_join(hru_grid_solrad, hru_statn_solrad, by = c('time', 'ID')) %>%
  mutate(diff = solrad_statn - solrad_grid,
         year = year(time)) %>%
  group_by(year, ID) %>%
  mutate(total_diff = cumsum(diff)) %>% 
  filter(time == max(time)) %>%
  group_by(ID) %>%
  summarize(solrad_avg_cumdiff = mean(total_diff)) %>%
  rename(value = ID)
...
tm_shape(hrus_map_solrad) +
  tm_polygons('solrad_avg_cumdiff', midpoint = NA, palette = 'YlOrRd', id = 'value') +
tm_shape(locations_solrad) +
  tm_symbols(col='blue', size=4)

```

Relative solar rad difference

```{r relative solrad, echo = F}

reg_solrad <- read_csv('C:/Users/sears/Documents/Repos/fourmile/HRU_output/reg_solradAll.csv',
                      skip = 14) %>%
  dplyr::select(-starts_with('@')) %>%
  mutate(time = dmy(time)) %>%
  summarize(across(`3`:`19`, ~ mean(.))) %>%
  pivot_longer(cols = `3`:`19`,
               names_to = 'value', 
               values_to = 'solrad_avg') %>%
  mutate(value = as.numeric(value))


hrus_map_solrad <- left_join(hrus_map_solrad, reg_solrad, by = 'value') 

hrus_map_solrad <- hrus_map_solrad %>%
  mutate(relative_avgcum_diff = solrad_avg_cumdiff / solrad_avg)

tm_shape(hrus_map_solrad) +
  tm_polygons('relative_avgcum_diff', midpoint = NA, palette = 'YlOrRd', id = 'value') +
tm_shape(locations_solrad) +
  tm_symbols(col='blue', size=4)

```

## Cumulative RH difference
...
rh_hrus <- left_join(hru_grid_rh, hru_statn_rh, by = c('time', 'ID')) %>%
  mutate(diff = rh_statn - rh_grid,
         year = year(time)) %>%
  group_by(year, ID) %>%
  mutate(total_diff = cumsum(diff)) %>% 
  filter(time == max(time)) %>%
  group_by(ID) %>%
  summarize(rh_avg_cumdiff = mean(total_diff)) %>%
  rename(value = ID)
...
tm_shape(hrus_map_rh) +
  tm_polygons('rh_avg_cumdiff', midpoint = NA, id = 'value') +
tm_shape(locations_rh) +
  tm_symbols(col='blue', size=4)

```

Relative RH difference

```{r relative rh, echo = F}

reg_rh <- read_csv('C:/Users/sears/Documents/Repos/fourmile/HRU_output/reg_rhumAll.csv',
                      skip = 14) %>%
  dplyr::select(-starts_with('@')) %>%
  mutate(time = dmy(time)) %>%
  summarize(across(`3`:`19`, ~ mean(.))) %>%
...
hrus_map_rh <- left_join(hrus_map_rh, reg_rh, by = 'value') 

hrus_map_rh <- hrus_map_rh %>%
  mutate(relative_avgcum_diff = rh_avg_cumdiff / rh_avg)

tm_shape(hrus_map_rh) +
  tm_polygons('relative_avgcum_diff', midpoint = NA, id = 'value') +
tm_shape(locations_rh) +
  tm_symbols(col='blue', size=4)

```

## Cumulative soil sat difference
...
soilSat_hrus <- left_join(hru_grid_soilSat, hru_statn_soilSat, by = c('time', 'ID')) %>%
  mutate(diff = soilSat_statn - soilSat_grid,
         year = year(time)) %>%
  group_by(year, ID) %>%
  mutate(total_diff = cumsum(diff)) %>% 
  filter(time == max(time)) %>%
  group_by(ID) %>%
  summarize(soilSat_avg_cumdiff = mean(total_diff)) %>%
  rename(value = ID)
...
tm_shape(hrus_map_soilSat) +
  tm_polygons('soilSat_avg_cumdiff', midpoint = NA, id = 'value') 
...
ET_hrus <- left_join(hru_grid_ET, hru_statn_ET, by = c('time', 'ID')) %>%
  mutate(diff = ET_statn - ET_grid,
         year = year(time)) %>%
  group_by(year, ID) %>%
  mutate(total_diff = cumsum(diff)) %>% 
  filter(time == max(time)) %>%
  group_by(ID) %>%
  summarize(ET_avg_cumdiff = mean(total_diff)) %>%
  rename(value = ID)
...
tm_shape(hrus_map_ET) +
  tm_polygons('ET_avg_cumdiff', midpoint = NA, id = 'value') 

```

## Cumulative wind difference

```{r wind, echo = F}

#get rid of a bunch of data
hru_grid_wind <- hru_grids %>%
  dplyr::select(c(time, ID, wind)) %>%
  rename(wind_grid = wind)

hru_statn_wind <- hru_statn %>%
  dplyr::select(c(time, ID, wind)) %>%
  rename(wind_statn = wind)

wind_hrus <- left_join(hru_grid_wind, hru_statn_wind, by = c('time', 'ID')) %>%
  mutate(diff = wind_statn - wind_grid,
         year = year(time)) %>%
  group_by(year, ID) %>%
  mutate(total_diff = cumsum(diff)) %>% 
  filter(time == max(time)) %>%
  group_by(ID) %>%
  summarize(wind_avg_cumdiff = mean(total_diff)) %>%
  rename(value = ID)

hrus_map_wind <- left_join(hrus, wind_hrus, by = 'value')
...
tm_shape(hrus_map_wind) +
  tm_polygons('wind_avg_cumdiff', midpoint = NA, id = 'value') +
tm_shape(locations_w) +
  tm_symbols(col='blue', size=4)
