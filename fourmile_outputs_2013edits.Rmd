...
- Dec-Mar: Winter
- Apr-Jun: Snowmelt/spring
- Jul-Aug: Monsoon
- Sep-Nov: Baseflow/fall
...
tmax_hrus_jan <- left_join(hru_grid_tmax_jan, hru_statn_tmax_jan, by = c('ID')) %>%
  mutate(diff = tmax_all_statn - tmax_all_grid) %>%
  rename(value = ID)

hrus_map_tmax_jan <- left_join(hrus, tmax_hrus_jan, by = 'value')

tmax_jan <- ggplot() +
  geom_sf(data = hrus_map_tmax_jan, aes(fill = diff), color = "black", lwd = 0.2) +
  scale_fill_viridis_c(option = "plasma", name = "tmax difference (°C)",
                       limits = c(-5.1, 3)) +
  theme_bw(base_size=13.5) +
  theme_void() +
...
tmax_hrus_apr <- left_join(hru_grid_tmax_apr, hru_statn_tmax_apr, by = c('ID')) %>%
  mutate(diff = tmax_all_statn - tmax_all_grid) %>%
  rename(value = ID)
...
tmax_apr <- ggplot() +
  geom_sf(data = hrus_map_tmax_apr, aes(fill = diff), color = "black", lwd = 0.2) +
  scale_fill_viridis_c(option = "plasma", name = "tmax difference (°C)",
                       limits = c(-5.1, 3)) +
  theme_bw(base_size=13.5) +
  theme_void() +
...
tmax_hrus_jul <- left_join(hru_grid_tmax_jul, hru_statn_tmax_jul, by = c('ID')) %>%
  mutate(diff = tmax_all_statn - tmax_all_grid) %>%
  rename(value = ID)

hrus_map_tmax_jul <- left_join(hrus, tmax_hrus_jul, by = 'value')

tmax_jul <- ggplot() +
  geom_sf(data = hrus_map_tmax_jul, aes(fill = diff), color = "black", lwd = 0.2) +
  scale_fill_viridis_c(option = "plasma", name = "tmax difference (°C)",
                            limits = c(-5.1, 3)) +
  theme_bw(base_size=13.5) +
  theme_void() +
...
tmax_hrus_sep <- left_join(hru_grid_tmax_sep, hru_statn_tmax_sep, by = c('ID')) %>%
  mutate(diff = tmax_all_statn - tmax_all_grid) %>%
  rename(value = ID)

hrus_map_tmax_sep <- left_join(hrus, tmax_hrus_sep, by = 'value')

tmax_sep <- ggplot() +
  geom_sf(data = hrus_map_tmax_sep, aes(fill = diff), color = "black", lwd = 0.2) +
  scale_fill_viridis_c(option = "plasma", name = "tmax difference (°C)",
                       limits = c(-5.1, 3)) +
  theme_bw(base_size=13.5) +
  theme_void() +
...
tmin_hrus_jan <- left_join(hru_grid_tmin_jan, hru_statn_tmin_jan, by = c('ID')) %>%
  mutate(diff = tmin_all_statn - tmin_all_grid) %>%
  rename(value = ID)

hrus_map_tmin_jan <- left_join(hrus, tmin_hrus_jan, by = 'value')

tmin_jan <- ggplot() +
  geom_sf(data = hrus_map_tmin_jan, aes(fill = diff), color = "black", lwd = 0.2) +
  scale_fill_viridis_c(option = "plasma", name = "tmin difference (°C)",
                       limits = c(-5, 3)) +
  theme_bw(base_size=13.5) +
  theme_void() +
...
tmin_hrus_apr <- left_join(hru_grid_tmin_apr, hru_statn_tmin_apr, by = c('ID')) %>%
  mutate(diff = tmin_all_statn - tmin_all_grid) %>%
  rename(value = ID)
...
tmin_apr <- ggplot() +
  geom_sf(data = hrus_map_tmin_apr, aes(fill = diff), color = "black", lwd = 0.2) +
  scale_fill_viridis_c(option = "plasma", name = "tmin difference (°C)",
                       limits = c(-5, 3)) +
  theme_bw(base_size=13.5) +
  theme_void() +
...
tmin_hrus_jul <- left_join(hru_grid_tmin_jul, hru_statn_tmin_jul, by = c('ID')) %>%
  mutate(diff = tmin_all_statn - tmin_all_grid) %>%
  rename(value = ID)

hrus_map_tmin_jul <- left_join(hrus, tmin_hrus_jul, by = 'value')

tmin_jul <- ggplot() +
  geom_sf(data = hrus_map_tmin_jul, aes(fill = diff), color = "black", lwd = 0.2) +
  scale_fill_viridis_c(option = "plasma", name = "tmin difference (°C)",
                            limits = c(-5, 3)) +
  theme_bw(base_size=13.5) +
  theme_void() +
...
tmin_hrus_sep <- left_join(hru_grid_tmin_sep, hru_statn_tmin_sep, by = c('ID')) %>%
  mutate(diff = tmin_all_statn - tmin_all_grid) %>%
  rename(value = ID)

hrus_map_tmin_sep <- left_join(hrus, tmin_hrus_sep, by = 'value')

tmin_sep <- ggplot() +
  geom_sf(data = hrus_map_tmin_sep, aes(fill = diff), color = "black", lwd = 0.2) +
  scale_fill_viridis_c(option = "plasma", name = "tmin difference (°C)",
                       limits = c(-5, 3)) +
  theme_bw(base_size=13.5) +
  theme_void() +
...
rd1_hru <- left_join(rd1_hru, elev_area, by = 'elevation_group') %>%
  mutate(RD1_mm_year = ((max_rd1_summed/1000)/group_area)/1000)
...
rd2_hru <- left_join(rd2_hru, elev_area, by = 'elevation_group') %>%
  mutate(rd2_mm_year = ((max_rd2_summed/1000)/group_area)/1000)
...
rg1_hru <- left_join(rg1_hru, elev_area, by = 'elevation_group') %>%
  mutate(rg1_mm_year = ((max_rg1_summed/1000)/group_area)/1000)
...
rg2_hru <- left_join(rg2_hru, elev_area, by = 'elevation_group') %>%
  mutate(rg2_mm_year = ((max_rg2_summed/1000)/group_area)/1000)
